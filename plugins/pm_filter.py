...
async def rename(bot,update):
	...
	await update.message.reply_text("»»——— 𝐏𝐥𝐞𝐚𝐬𝐞 𝐄𝐧𝐭𝐞𝐫 𝐧𝐞𝐰 𝐅𝐢𝐥𝐞 𝐍𝐚𝐦𝐞...",	
...
async def doc(bot, update):
    try:
        ...
        await ms.edit("三 ᴘʀᴇᴘᴀʀɪɴɢ ᴛᴏ ʀᴇᴄᴇɪᴠᴇ ꜰɪʟᴇ...︻デ═一")
        c_time = time.time() 
        try:
           if type == "document":
              await bot.send_document(
	            update.message.chat.id,
                       ...
                       caption=caption, 
                       progress=progress_for_pyrogram,
                       progress_args=( "**⎝⎝✧ ʀᴇᴄɪᴇᴠɪɴɢ ꜰɪʟᴇ ꜰʀᴏᴍ ɴɪᴍꜱᴀʀᴀ ꜱᴇʀᴠᴇʀ ✧⎠⎠**",  ms, c_time))
           elif type == "video": 
...
async def next_page(bot, query):
    ...
    btn.insert(0,
        [ 
	    InlineKeyboardButton(text="♂️ ɪɴꜰɪɴɪᴛʏ ᴍᴏᴠɪᴇꜱ ♂️", url='https://t.me/sinhalafilx'),
        ] 
    )
...
async def advantage_spoll_choker(bot, query):
    ...
    await query.answer('ᴄʜᴇᴄᴋɪɴɢ ɪɴꜰɪɴɪᴛʏ ᴅᴀᴛᴀʙᴀꜱᴇ ...')
    ...
    if k == False:
        ...
        if files:
            ...
            k = await query.message.edit('😶 සොරි අනේ මෙකනම් දැනට අපේ database එකේ නැ  !')
...
async def languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="👇 ඔයාට ඔනි භාෂාව තෝරගන්නකෝ 👇", callback_data="ident"
            )
        ],
    )
    req = query.from_user.id
    offset = 0
    btn.append([InlineKeyboardButton(text="↭ ආපසු ​↭", callback_data=f"fl#homepage#{key}")])
...
async def filter_languages_cb_handler(client: Client, query: CallbackQuery):
    try:
        ...
        if not files:
            await query.answer("🚫 මෙහෙම File එකක් නැ අනේ 🚫", show_alert=1)
            return
        temp.GETALL[key] = files 
...
async def cb_handler(client: Client, query: CallbackQuery):
        ...
        if chat_type == enums.ChatType.PRIVATE:
            ...
            if grpid is not None:
                ...
                try:
                    ...
                    return await query.answer('♥️ Love @Nimsar_a ♥️')
            else:
                await query.message.edit_text(
                    "I'm not connected to any groups!\nCheck /connections or connect to any groups",
                    quote=True
                )
                return await query.answer('♥️ Thank You Nimsara ♥️')
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton('♂️ ꜱᴇᴀʀᴄʜ  ♂️', switch_inline_query_current_chat=''),
            InlineKeyboardButton('✨️ ᴜᴘᴅᴀᴛᴇꜱ ✨️', url='https://t.me/sinhalafilx')
            ],[
            InlineKeyboardButton('⭕ ʜᴇʟᴘ ⭕', callback_data='help'),
            InlineKeyboardButton('🌸 ᴀʙᴏᴜᴛ ᴍᴇ 🌸', callback_data='about')
        ],[
            InlineKeyboardButton('♀️ᴍᴏʀᴇ ʜᴇʟᴘ♀️', callback_data='leech_url_help'),
            InlineKeyboardButton('🔰 ᴏᴘᴇɴ ꜱᴇᴛᴛɪɴɢꜱ 🔰 ', callback_data='openSettings'),
            ],
        [
            InlineKeyboardButton('⪦ ᴊᴏɪɴ ᴛᴠ ꜱᴇʀɪᴇꜱ ᴄʜᴀɴɴᴇʟ  ⪧', url='https://t.me/infinitytvseries')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        await query.answer('♂️ Thank You Nimsara ♂️')
        ...
        buttons = [[
            InlineKeyboardButton('✨️ ᴜᴘᴅᴀᴛᴇꜱ  ✨️', url='https://t.me/sinhalafilx'),
            InlineKeyboardButton('♀️ Nimsara', callback_data='source')
        ], [
            InlineKeyboardButton('🏠 Home', callback_data='start'),
        ...
        await query.answer("Thank You Nimsara")
        ...
        try:
            btn = [[
                ...
                InlineKeyboardButton(text=f"🐞 REPORT BUG 🐞", url=f"https://telegram.me/infinityworldlk")
            ],[
                InlineKeyboardButton(text=f"⚡️ Update Channel ⚡️", url=f"https://t.me/sinhalafilx")
            ...
            reply_markup = InlineKeyboardMarkup(btn)
            await client.send_message(int(user_id), f"😒 oops! Beauty, ඔයා ඉල්ලපු  `{movie}` දැනටනම් අපි ලග නැ, we are really trying our best to serve you this content, can you please provide us some more details related to your query `{movie}`, \nSend details to Admin : <a href='https://telegram.me/{ADMIN_USRNM}'>**Send here...**</a>\n\n❤ Thank You for the contribution", reply_markup=reply_markup)
        ...
        try:
            ...
            reply_markup_lzdv = InlineKeyboardMarkup(btn_lzdv) 
            reply_markup = InlineKeyboardMarkup(btn)
            await client.send_message(int(user_id), f"✅ Hey Beauty, Your requested content named `{movie}` is now available in our database! You can easily get this movie by searching it's correct name in our official group...\n\n❤ Thank You for the contribution", reply_markup=reply_markup)
            await query.edit_message_text(text=f"- __**User notified successfully sweetie...✅**__\n\n⏳**Status** : Upload done ✅.\n🪪**UserID** : `{user_id}`\n🎞**Content** : `{movie}`\n\n\n🦋", reply_markup=reply_markup_lzdv)
        # Delete callback query message
            await query.answer()
            await query.delete()
    ...
    elif query.data == "donatenimsaradev":
        buttons = [
            [ InlineKeyboardButton("⨳   Close   ⨳", callback_data="close_data") ]
            ]
        reply_markup = InlineKeyboardMarkup(buttons)
...
async def auto_filter(client, msg, spoll=False):
    ...
    fetching_message = await message.reply_text(f"හොයන ගමන් ඉන්නේ නිම්සර database එකේන් {full_name}...")
    ...
    if imdb:
        ...
        cap = f"⚡hey Beauty බලන්න ඔයා හොයන එක තියෙද කියලා {search}"
...
async def advantage_spell_chok(msg):
    ...
    gs_parsed = []
    if not g_s:
        k = await msg.reply("මට මේ නමින් කිසිම දෙයක් හොයාගන්න බැ .")
        await asyncio.sleep(10)
        await k.delete()
        return
    ...
    if not movielist:
        k = await msg.reply("Hey beauty! The requested content is currently unavailable in our database, have some patience 🙂 - our great admin will upload it as soon as possible \n\n               **or**\n\nDiscuss issue with admin here 👉  <a href='https://t.me/infinityworldlk'>Discuss Here</a> ♥️ ")
        await asyncio.sleep(10)
        await k.delete()
        return
